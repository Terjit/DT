...
class updateActual(QMainWindow):
    ...
    def importCsv(self):
        button = QMessageBox.question(self, "Confirm Update Actual", "Updating the actual well path will remove the current actual well path.\n"
        ...
        cursor = conn.cursor()
        cursor.execute("DELETE FROM DEV WHERE wellName = ? AND Planned = 0",(self.well,))
