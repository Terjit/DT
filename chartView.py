...
class chartView(QMainWindow):
    ...
    def pullData(self):
        ...
        cursor = conn.cursor()      
        data = cursor.execute(f"SELECT * FROM DEV WHERE wellName = '{self.well}' AND Planned = 0")
        if data.fetchone() is not None:
            self.wellDf = pd.read_sql_query(f"SELECT * FROM DEV WHERE wellName = '{self.well}' AND Planned = 0 AND Lateral = 'NULL'", conn)
        else:
    ...
    def threed(self):
        ...
        self.ax3d.plot3D(self.wellDf['EW'], self.wellDf['NS'],self.wellDf['TVD'], color='black')
